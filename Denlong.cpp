...
int main() {
    ...
    for (int i = 1; i < n; i++) {
        double distance = lanterns[i] - lanterns[i - 1];
        maxDistance = max(maxDistance, distance);
    }

    double edgeToLeft = lanterns[0] - 0;
    double edgeToRight = l - lanterns[n - 1];

    double radius = max({edgeToLeft, edgeToRight, maxDistance / 2});
